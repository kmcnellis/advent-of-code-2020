...
fs.readFile('input.txt', 'utf8', function(err: Error, contents: string) {
  ...
  const dangerous = Object.values(ingredients).filter(x => x.isAllergen()).sort(compare).map(x => x.name).reduce((c, s) => c + "," + s);
  console.log("dangerous", dangerous);

});

const compare = (a: Ingredient, b: Ingredient): number => {
